...
export const handleImageUpload = async (
  ...
  if(error && error.statusCode!='409'){
    ...
    throw new Error('unable to upload file, try again later')
  }
  // @ts-ignore
  if(data || error.statusCode === '409'){
    const {data: {publicUrl}} = data ? await supabase.storage.from('images').getPublicUrl(data.path) : await supabase.storage.from('images').getPublicUrl(file.name)
