...
import {ChevronDown,LoaderCircleIcon, Trash2} from "lucide-react";
...
import {columns, TableArticle} from "@/app/admin/posts/table/columns";
import {Article} from "@/app/admin/posts/interfaces";
import {flexRender, useReactTable} from "@tanstack/react-table";
import {getArticles} from "@/actions/articles/categories";
...
export default function PostsTable({className,}: { className?: string, }) {
    ...
    useEffect(()=>{
        if(state?.success){
            ...
            setArticles((prev)=>{return prev.filter(a=>{
    ...
    return (
        <div className="relative w-full">
            {pending && <LoadingSpinner className={'z-[99] absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]'}/>}
            ...
            <div className="flex items-center py-4">
                ...
                <form action = {action}>
                    <Button type = 'submit' className={'ml-4 text-[12px] font-bold flex items-center text-muted'}><Trash2 className={'mr-2 text-muted'}/>Delete selected</Button>
                </form>
            </div>
